...
const ShoppingList: FC<ShoppingListProps> = ({ plan, manualItems, onAddManualItem, onDeleteManualItem }) => {
  ...
  // Derive the deduplicated + counted list from the shared utility
      ...
      <CardContent className="flex-grow space-y-4">
        ...
        {hasManualItems && (
          <div>
            ...
            {/* Scroll only when the list gets long */}
            <ScrollArea className={manualItems.length > 5 ? "h-32" : ""}>
              <ul className="space-y-1">
                {manualItems.map(item => (
                  <li
                    key={item.id}
                    className="flex justify-between items-center text-foreground shopping-list-item manual-grocery-item py-1"
                  >
                    <span>{item.name}</span>
                    <Button
                      variant="ghost"
